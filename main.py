...
async def tweet(update: Update, context: CallbackContext) -> None:
  args = context.args
  if len(args) < 2:
    await update.message.reply_text('Usage: /tweet {accessToken} {text}')
    return

  access_token = args[0]
  tweet_text = ' '.join(arg.strip() for arg in args[1:]).replace('\\\\n', '\n')
...
async def delete(update: Update, context: CallbackContext) -> None:
  args = context.args
  if len(args) < 2:
    await update.message.reply_text('Usage: /delete {accessToken} {id}')
    return
  ...
  if response.status_code == 200:
    await context.bot.send_message(chat_id=chatId, text='❌ Tweet Deleted ❌')
  else:
    print(response_data)
    await update.message.reply_text(
        f'🚫 Deletion Failed 🚫\nError: {response_data["title"]}')
